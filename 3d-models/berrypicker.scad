...
plungerh=handlel-clippullh-clipplungerzmovement; // height;
plungerd=clipslided;
...
plungercountersupporth=clipslided+plungerangledh+clipplungerzmovement;
plungercountersupportw=2;
plungercountersupportheight=plungerd+clipplungerzmovement+plungercountersupportw;
longhandlel=500;
longhandled=handled;
...
handlelowh=clippullh+plungerd+clipplungerzmovement+5;
...
module plunger() {
  ...
  difference() {
    union() {
      translate([0,0,clipslided]) cylinder(h=plungerh,d=plungerd);
      translate([-plungerd/2-plungeclipd+1,-plungeclipw/2+plungeclipcut,clipslided+plungerh-plungeclipd]) triangle(plungeclipd,plungeclipw-plungeclipcut*2,plungeclipd,3);
      

      intersection() {
	difference() {
	  //	  translate([plungeclipfingerd*(2/3),0,clipslided+plungerh+plungeclipfingerd/2+2]) sphere(plungeclipfingerd+wall);
	  //	  translate([plungeclipfingerd*(2/3),0,clipslided+plungerh+plungeclipfingerd/2+2]) sphere(plungeclipfingerd);
	}
	translate([plungeclipfingerd*(2/3)-plungerd,-plungerd*2/2,clipslided+plungerh-clipplungerzmovement]) cube([plungerd,plungerd*2,clipplungerzmovement]);
      }
    }
    ...
    // text cut
    hull() {
      translate([plungerd-0.01,-plungerd/2,clipslided+plungerangledh+ztolerance+4-plungerd*2/3]) cube([0.02,plungerd,tl]);
      translate([plungerd/3,-plungerd/2,clipslided+plungerangledh+ztolerance+4]) cube([plungerd/3,plungerd,tl]);
    }
    translate([plungerd/3-textdepth+0.01,0,clipslided+plungerangledh+ztolerance+4+tl/2]) rotate([0,90,0]) linear_extrude(height=textdepth) text(versiontextb,font="Liberation Sans:style=Bold",size=((print==4)?textsize/2:textsize),halign="center", valign="center");

    
    translate([-plungerd/2,-plungerd/2,clipslided-0.02]) triangle(plungerd+0.01,plungerd,plungerangledh+0.02,0);
    ...
    union() {
      ...
      translate([-plungerd/2+plungeclipdepth-plungeclipnotchd+plungecliptopadjust,-plungeclipw/2+plungeclipcut-0.1,clipslided+plungerh]) rotate([-90,0,0]) cylinder(d=plungeclipnotchd,h=plungeclipw-2*plungeclipcut+0.2,$fn=6);
    }

    // Cut for finger point
    //   translate([plungeclipfingerd*(2/3),0,clipslided+plungerh+plungeclipfingerd/2+5]) sphere(plungeclipfingerd);
  }
}
...
module berrypicker() {
  ...
  difference() {
	...
	translate([-storageh,0,storagel-storageh-wall]) roundedbox(wall,width,wall,cornerd);
      ...
      for (y=[0:fingerdistance:width-fingerdistance]) {
	hull() {
	...
	hull() {
	  translate([wall/2,y+wall/2,storagel+preventerh*2+wall/2]) sphere(d=wall);
	  translate([preventerh+wall/2,y+wall/2,storagel+preventerh+wall]) sphere(d=wall);
	  translate([wall/2,y+fingerdistance/2,storagel+preventerh*3+wall/2]) sphere(d=wall);
	  translate([preventerh+wall/2,y+fingerdistance/2,storagel+preventerh*2+wall]) sphere(d=wall);
	}
	hull() {
	  ymax=(y+fingerdistance+wall/2>=width?-wall:0);
	  translate([wall/2,y+wall/2+fingerdistance+ymax,storagel+preventerh*2+wall/2]) sphere(d=wall);
	  translate([preventerh+wall/2,y+fingerdistance+wall/2+ymax,storagel+preventerh+wall]) sphere(d=wall);
	  translate([wall/2,y+fingerdistance/2,storagel+preventerh*3+wall/2]) sphere(d=wall);
	  translate([preventerh+wall/2,y+fingerdistance/2,storagel+preventerh*2+wall]) sphere(d=wall);
	}
      }
  ...
  difference() {
    ...
    translate([height-wall/2,width/2-clipw/2,clippullh]) triangle(wall/2+0.01,clipw,wall/2,0); // DUPLICATED in handle
  }
    
  // Clip to keep back closed
  difference() {
    hull() {
      translate([height-wall-clipdepth+plungerx,width/2-clipw/2+cutw,0]) roundedbox(clipdepth+wall/2,clipw-cutw*2,clipheight,cornerd);
      translate([height-wall-clipdepth+plungerx,width/2-cornerd/2,wall+clipw/2]) roundedbox(clipdepth-0.5+cornerd,cornerd,cornerd,cornerd  );
    }
    translate([height-wall-xtolerance-0.5,width/2-clipw/2,clippullh]) triangle(0.5+xtolerance,clipw,clipheight+clipw/2-clippullh+cornerd,3);
  }
  ...
  intersection() {
	...
	translate([height+handlex+plungerx,width/2,0]) cylinder(h=clippullh,d1=clipw-2*clippullh-2*cutw, d2=clipslided);
      ...
      difference() {
	translate([height+handlex+plungerx,width/2,clippullh-0.1]) cylinder(h=clipslided+plungerangledh+clipplungerzmovement+0.1,d=clipslided);
	translate([height+handlex+plungerx-clipslided/2,width/2-clipslided/2,clippullh+plungerd+clipplungerzmovement]) triangle(clipslided+0.01,clipslided,plungerangledh+0.01,1);
      }

      translate([height+plungerx+xtolerance,width/2-clipslided/2,clippullh]) triangle(handlex-clipslided-xtolerance-xtolerance,clippullh,clippullh,11); // clipw/2-clippullh-xtolerance+cutw,clippullh+cutw-ztolerance,10);
      translate([height+plungerx+xtolerance,width/2+clipslided/2-clippullh,clippullh]) triangle(handlex-clipslided-xtolerance-xtolerance,clippullh,clippullh,8);
  
    }
    hull() {
      translate([height+plungerx+handlex-clipdepth-xtolerance,width/2,0]) cylinder(h=clippullh+clipslided+plungerangledh+clipplungerzmovement,d=clipslided);
  ...
  translate([height+plungerx+handlex-plungerd/2-plungercountersupportheight/2+plungercountersupportw/2,width/2-plungercountersupportw/2,clippullh-0.01]) triangle(plungercountersupportheight/2,plungercountersupportw,plungercountersupportheight,0);
  ...
  union() {
	...
	hull() {
      ...
      translate([height-wall/2,width/2-clipw/2,clippullh]) triangle(wall/2+0.01,clipw,wall/2,0); // DUPLICATED in top
...
intersection() {
  ...
  union() {
    ...
    if (print==0) {
      translate([height+handlex,width/2,clippullh+clipplungerzmovement+ztolerance+plungerz]) plunger();
    }
  
    if (print==2 || print==3 || print==4) {
      translate([height,width+1+clipslided/2,plungerh+clipslided]) rotate([180,0,120]) plunger();
    }
  }
}
