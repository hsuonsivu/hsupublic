...
module pussinsulkija(l=250) {
  ...
  if (1) difference() {
	...
	translate([outsided/2+xtolerance,ytolerance,-h/2]) roundedbox(rootl,w-ytolerance,h,cornerd,1);
      ...
      translate([0,bagw/2,-h/2]) roundedbox(l+xtolerance+springw-springoutd/2+cornerd/2,w-bagw/2,h,cornerd,1);
      translate([0,bagw/2,-h/2]) roundedbox(l,w-bagw/2-springw-springcutw,h,cornerd,1);
	...
	translate([l+xtolerance+springw-springoutd/2,w-springoutd/2,-h/2]) cube([springoutd/2,springoutd/2,h]);
      ...
      translate([l+xtolerance,-w*2,-h/2]) roundedbox(springw,w*3-springoutd/2+cornerd/2,h,cornerd,1);
	  ...
	  difference() {
	    union() {
	      for (y=[-w,cornerd]) {
		hull() {
		  translate([l+xtolerance-(y==cornerd?ytolerance+cornerd:0),y-springhook-springw-ytolerance-springcutw-cornerd-0.1,-h/2]) roundedbox(cornerd,springhook+springw+springcutw+cornerd+0.1,cornerd,cornerd,1);
		  translate([l+xtolerance-springcutw-springhook,y-ytolerance-dw,-h/2]) roundedbox(springw,dw,cornerd,cornerd,1);
		  translate([l+xtolerance,y-springhook-springw-ytolerance-springcutw-cornerd-0.1,-cornerd/2]) roundedbox(cornerd,springhook+springw+springcutw+cornerd,cornerd,cornerd);
		  translate([l+xtolerance-springcutw-springhook,y-cornerd-ytolerance,-cornerd/2]) roundedbox(springw,cornerd,cornerd,cornerd);
		}
	      }

	      translate([l+xtolerance-springcutw-springhook,-cornerd/2,-h/2]) roundedbox(springhook+springcutw-xtolerance,cornerd*2,h,cornerd,1);
	    }

	    translate([l,-w-0.01,-h/2-0.01]) cube([xtolerance,w+cornerd+0.02,h+0.02]);
	  }
	}
    }
	...
	intersection() {
	  union() {
	    translate([l-(springcutd-springcutw*2)/2,w-springw-springcutd/2-0.01,-h/2-0.01]) cube([springcutd/2,springcutd/2+0.01,h+0.02]);
	    translate([l+xtolerance-springcutd/2,w-springw-springcutw,-h/2-0.01]) cube([springcutw,springcutd/2,h+0.02]);
	  }
  ...
  if (1) rotate([0,0,-printangle]) difference() {
	...
	translate([outsided/2+xtolerance,-w,-h/2]) roundedbox(l-(outsided/2+xtolerance)-springhook-cornerd,w-bagw/2,h,cornerd,1);
	...
	hull() {
	  translate([l-springhook-cornerd-cornerd,-w,-cornerd/2]) roundedbox(cornerd,w-bagw/2,cornerd,cornerd,1);
	  translate([l-springhook-cornerd-cornerd,-w,-cornerd/2]) roundedbox(springhook+springcutw+cornerd,cornerd,cornerd,cornerd,1);
	  translate([l-springhook-cornerd-cornerd,-w,-h/2]) roundedbox(cornerd,cornerd,h,cornerd,1);
	}
      }
    }
}
